...
import wandb
import pandas as pd
from transformers import MT5Tokenizer, DataCollatorForSeq2Seq, AutoModelForSeq2SeqLM, AdamW, T5Tokenizer, MBart50TokenizerFast, MBartForConditionalGeneration
import evaluate
from accelerate import Accelerator

def load_tokenizer(local_tokenizer_path = '/home/msiepel/mbart_tokenizer'):
    tokenizer = MBart50TokenizerFast.from_pretrained(local_tokenizer_path)
    return tokenizer
...
def preprocess_function(examples, tokenizer, max_length_sentence):
    # Prepend the Dutch language code to both inputs and targets
    inputs = ["nl_XX " + ex for ex in examples["Conclusie"]]
    targets = ["nl_XX " + ex for ex in examples["Codes"]]
...
def freeze_layers(model, freeze_all_but_x_layers):
    ...
    for i in range(-freeze_all_but_x_layers, 0):
...
def setup_model(tokenizer, freeze_all_but_x_layers, local_model_path = '/home/msiepel/models/mbart-large-50'):
...
def train_model(model, optimizer, accelerator, max_generate_length, train_dataloader, eval_dataloader, test_dataloader, num_train_epochs, tokenizer, run_name, patience=3):
    ...
    for epoch in range(num_train_epochs):
        ...
        if eval_metrics["loss"] < lowest_loss:  # Update loss accordingly
            ...
            torch.save(model.state_dict(), f'/home/msiepel/models/trained_models/{run_name}.pth')  # Save the model weights
            early_stopping_counter = 0
        else:
    ...
    artifact.add_file(f'/home/msiepel/models/trained_models/{run_name}.pth')
